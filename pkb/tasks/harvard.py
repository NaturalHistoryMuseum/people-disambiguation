...
class HarvardIndexSearchTask(BaseTask):
    ...
    def _search(self):
        ...
        df = pd.concat(df)
        # Extract unique agentid values as a list
        unique_agentids = df['agentid'].unique()
        # Convert to a Python list
        unique_agentids_list = unique_agentids.tolist()
        return unique_agentids_list
        
    def run(self):
        logger.info(f'Parsing Harvard Index of unique persons (or groups)')         
        collector_ids = set(self._search()) 
        logger.info(f'%s unique persons (or groups) retrieved from Harvard Index', len(collector_ids)) 
        with self.output().open('w') as f:                  
            f.write(yaml.dump(list(collector_ids),  default_flow_style=False))         
...
class HarvardIndexDetailTask(BaseTask):
    ...
    def run(self):
        ...
        with open(self.output().path, 'w', encoding='utf-8') as outf:
            
            start_time = time.time()  
            # collector_ids.reverse()
            # collector_ids = [42819]
            
            # temp_file = self.output().temporary_path()  # added Luigi atomic file handling
            
            for collector_id in tqdm(collector_ids):              
                collector = self._parse_detail_page(collector_id)
                if collector is None:
                    # Log an error or handle the issue gracefully
                    logger.info(f"Warning: Collector data for ID {collector_id} could not be parsed.")
                    continue  # Skip to the next collector_id
                ...
                # Write each collector's data as YAML into the temporary file
...
class HarvardIndexCollectorsTask(BaseTask):
    ...
    def run(self):
        ...
        with open(HarvardIndexDetailTask().output().path, 'r', encoding='utf-8') as f:
            # collector_ids = yaml.full_load(f)  
